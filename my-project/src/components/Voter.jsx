  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const electionsRes = await axios.get('http://localhost:7075/api/elections/available');
        setAvailableElections(electionsRes.data);

        // Fetch voter's registrations
        const regRes = await axios.get(`http://localhost:7075/api/voters/${userData.email}/registrations`);
        ...
        const historyRes = await axios.get(`http://localhost:7075/api/voters/${userData.email}/history`);
        setVotingHistory(historyRes.data);

        // Fetch completed elections with results
        const completedRes = await axios.get('http://localhost:7075/api/elections/completed');
...
const handleRegisterForElection = async (electionId, isRequest) => {
  ...
  try {
    await axios.post(
      `http://localhost:7075/api/elections/${electionId}/register${isRequest ? '?isRequest=true' : ''}`,
      {
 ...
 const handleCastVote = async (election) => {
  try {
    ...
    const response = await axios.get(`http://localhost:7075/api/elections/${checkId}/candidates`);
  ...
  const handleSubmitVote = async () => {
    ...
    try {
      await axios.post(`http://localhost:7075/api/elections/${selectedElection.id}/vote`, {
        voterEmail: userData.email,
