...
const openVoterModal = () => {
  ...
  const fetchResults = async () => {
    try {
      const response = await axios.get('http://localhost:7075/api/election-results/all');
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        const response = await fetch("http://localhost:7075/election-requests");
...
useEffect(() => {
  const fetchOrganizers = () => {
    axios.get("http://localhost:7075/users/organizers")
      .then(res => {
...
useEffect(() => {
  const fetchVoters = () => {
    axios.get("http://localhost:7075/users/voters")
      .then(res => {
...
useEffect(() => {
  const fetchActiveElections = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:7075/active-elections');
...
const fetchArchivedElections = async () => {
  ...
  const handleApproveRequest = async (requestId) => {
    ...
    try {
      const response = await fetch(`http://localhost:7075/election-requests/${requestId}/approve`, {
        method: 'PUT',
  ...
  const handleRejectRequest = async (requestId) => {
    ...
    try {
      const response = await fetch(`http://localhost:7075/election-requests/${requestId}/reject`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
 ...
 const handleToggleOrganizerStatus = async (email) => {
  try {
    const response = await axios.put(`http://localhost:7075/users/${email}/toggle-status`);
...
const handleAddOrganizer = async (formData) => {
    try {
      const response = await axios.post("http://localhost:7075/users/organizers", formData);

      if (response.status === 200 || response.status === 201) {
        const res = await axios.get("http://localhost:7075/users/organizers");
        const transformedOrganizers = res.data.map(organizer => ({
          id: organizer.email,
          name: organizer.fullName,
          email: organizer.email,
 ...
 const handleAddVoter = async (formData) => {
    try {
      const response = await axios.post('http://localhost:7075/users/voters', formData);
      
      if (response.status === 200 || response.status === 201) {
        const res = await axios.get("http://localhost:7075/users/voters");
        const transformedVoters = res.data.map(voter => ({
          id: voter.email,
          name: voter.fullName,
          email: voter.email,
  ...
  try {
    const response = await axios.get(`http://localhost:7075/users/${user.email}`);
    const fetchedUser = {
      ...response.data,
