...
const [showAddCandidate, setShowAddCandidate] = useState(false);
  ...
  const fetchResults = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:7075/api/election-results/organizer/${userData.fullName}`
      );
  ...
  const fetchVotersForMyElections = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:7075/api/organizers/${userData.fullName}/registered-voters`
      );
  ...
  const fetchCandidatesForMyElections = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:7075/api/organizers/${userData.fullName}/candidates`
      );
  ...
  useEffect(() => {
    ...
    try {
      const response = await axios.get('http://localhost:7075/election-requests');
  ...
  const response = await axios.get(
    `http://localhost:7075/active-elections/organizer/${userData.fullName}`
  );
 ...
 const handleStartElection = async (requestId) => {
  ...
  try {
    await axios.post(`http://localhost:7075/active-elections/start/${requestId}`);
    alert('Election started successfully!');
    
    // Refresh data
    const [approvedRes, activeRes] = await Promise.all([
      axios.get('http://localhost:7075/election-requests'),
      axios.get(`http://localhost:7075/active-elections/organizer/${userData.fullName}`)
    ]);
  ...
  try {
    // End the election
    await axios.put(`http://localhost:7075/active-elections/${electionId}/end`);
  ...
  const handleSubmitRequest = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:7075/election-requests", requestData);
      alert('Election request submitted successfully!');

      const updatedRequests = await axios.get('http://localhost:7075/election-requests');
  ...
  try {
    await axios.post(
      `http://localhost:7075/api/elections/${newCandidate.electionId}/candidates`,
      {
